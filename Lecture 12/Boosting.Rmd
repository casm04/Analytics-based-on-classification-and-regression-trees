...
# boosted tree model 
biomass_bt <-boost_tree(learn_rate=0.3, trees = 100, tree_depth= 6, min_n=1, sample_size=1, mode="regression") %>% set_engine("xgboost", verbose=2) %>%    fit(HHV ~ ., data = juice(biomass_tr))
```
```{r}
predict(biomass_tree, head(biomass_te, 5))
```
```{r}
varImpPlot(biomass_tree)
```

generate the predicted value
```{r}
# predict(bag.boston, head(Boston, 5))
pred_  = predict(biomass_tree, biomass_te)
#pred
```
# regression compare Testig vs Predicted value
```{r}
biomass_te$pred = pred_
g <- ggplot(biomass_te, aes(x = HHV , y = pred)) +
  geom_point() +  geom_line(aes(x = pred , y = pred))
g
```
...
```{r}
# Another xgboost model
iris_fit <- boost_tree(mode = "classification", trees = 20) %>%
  set_engine("xgboost") %>%
